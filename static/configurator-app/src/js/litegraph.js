...
function LGraph(o, apiClient, logger = (type, text) => console.log(text, {type})) {
            ...
            const promises =
                [
                    new Promise(async resolve => {
                        (await this.apiClient.call("/rest/api/3/issuetype", "GET"))
                            .forEach(issueType => this.jiraSpecifics.typeMap[issueType.name] = issueType.id);
                        resolve();
                    }),
                    new Promise(async resolve => {
                        (await this.apiClient.call("/rest/api/3/project/search", "GET")).values
                            .forEach(project => this.jiraSpecifics.projects.push({name: project.name, id: project.id}));
                        resolve();
                    }),
                    new Promise(async resolve => {
                        this.jiraSpecifics.assignees = (await this.apiClient.call("/rest/api/3/users/search", "GET"))
                            .filter(assignee => assignee.accountType === "atlassian");
                        resolve();
                    })
                ];
    ...
    for (var i = 0; i < widgets.length; ++i) {
        ...
        switch (w.type) {
            ...
            case "toggle":
                ...
                if (show_text && !w.disabled)
                    ctx.stroke();
                ctx.fillStyle = w.value ? "#0052CC" : "#333";
                ctx.beginPath();
...
LGraphCanvas.prototype.prompt = function (title, value, callback, event, multiline, inputType = 'text') {
    ...
    if (multiline)
        ...
        dialog.innerHTML = "<span class='name'></span> <input autofocus type='" + inputType + "' class='value'/><button class='rounded'>OK</button>";
...
LGraphCanvas.prototype.getCanvasMenuOptions = function () {
    ...
    //Forces the nodes menu directly
    //return LGraphCanvas.onMenuAdd();
